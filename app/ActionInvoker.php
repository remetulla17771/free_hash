...
final class ActionInvoker
{
    ...
    public function invoke(object $controller, string $method, array $parameters = []): mixed
    {
        if (!method_exists($controller, $method)) {
            throw new RuntimeException("Action not found: " . $controller::class . "::{$method}().", 404);
        }

        $reflection = new ReflectionMethod($controller, $method);
        if (!$reflection->isPublic()) {
            throw new RuntimeException("Action is not public: " . $controller::class . "::{$method}().", 404);
        }
        ...
        foreach ($reflection->getParameters() as $parameter) {
            ...
            if (array_key_exists($name, $parameters)) {
                $arguments[] = $this->resolveRouteParameter($parameters[$name], $type, $name);
                continue;
            }
    ...
    private function resolveRouteParameter(mixed $value, ?\ReflectionType $type, string $name): mixed
    {
        if (!$type instanceof ReflectionNamedType || !$type->isBuiltin()) {
